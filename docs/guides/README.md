...
所有指南均以当前实现为界。诸如 Markdown 表题/表注字体的独立配置目前不是已提供的功能；如未来实现，会另行在发布说明和对应指南中标明。（摘要独立段距已于 2026-08-21 实现为 `titleAbstractSkip` / `abstractKeywordsSkip`；`pgfplots` 已于 2026-08-25 由 Profile 条件加载并纳入回归，见 TikZ / PGF 指南。）
