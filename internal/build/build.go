...
func runWithExecutorAndResources(ctx context.Context, projectDir string, executor commandExecutor, scriptPath, profileDir string) Result {
	...
	if _, err := os.Stat(tmpPDF); err != nil {
		result.Diagnostics = append(result.Diagnostics, missingPDFDiagnostic(xelatexAvailable(scriptPath)))
		return result
	}
...
// missingPDFDiagnostic explains an absent PDF, naming the missing TeX
// distribution when that is the reason.
//
// It has to name it, because doctor now reports a missing XeLaTeX as a warning
// rather than a failure: a machine with no TeX is a usable NodePaper
// installation for everything except this stage. That is only safe if the
// stage that does need TeX says so plainly when it is reached. The transition
// script deliberately writes the LaTeX and exits 0 when it finds no compiler,
// so without this the user saw "PDF was not generated by the PowerShell build"
// and an invitation to read two logs - the one situation where doctor says
// "fine" and the build must not be cryptic.
func missingPDFDiagnostic(xelatexFound bool) diagnostic.Diagnostic {
	if !xelatexFound {
		return diagnostic.Diagnostic{
			Severity: diagnostic.SeverityError,
			Code:     "NP6002",
			Message: "no PDF was produced because XeLaTeX was not found; the LaTeX was generated " +
				"and only the PDF stage needs a TeX distribution",
			Suggestion: "Install a TeX distribution, then open a NEW terminal (PATH changes do not " +
				"affect already-open windows) and run `nodepaper doctor`:\n" +
				"  MiKTeX     https://miktex.org/download (~140 MB download, ~1 GB installed)\n" +
				"  TeX Live   https://tug.org/texlive/windows.html (~6.3 GB download, ~8-9 GB installed)\n" +
				"Until then, `nodepaper export` writes a LaTeX project that compiles on a machine " +
				"that has one.",
			Source: "build",
		}
	}
	return diagnostic.Diagnostic{
		...
		Suggestion: "Check the PowerShell and LaTeX logs; XeLaTeX was found, so this is a compile failure rather than a missing TeX installation.",
		Source:     "build",
	}
}

// xelatexAvailable reports whether a XeLaTeX the build could have used exists.
// It repeats the search order of Build-Paper.ps1 - the TeX bundled beside the
// script first, then PATH - so that the diagnostic above describes the same
// machine the script saw rather than a second, differently-resolved one.
func xelatexAvailable(scriptPath string) bool {
	if scriptPath != "" {
		bundled := filepath.Join(filepath.Dir(scriptPath), "tools", "windows-x64", "texlive", "bin", "windows", "xelatex.exe")
